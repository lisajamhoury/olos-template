...
// 2D simplex noise
noise.simplex = function (xin, yin) {
 ...
 *     `options.ySegments + 1`.
 */
// export function Terrain(options, THREE) {
THREE.Terrain = function (options) {
  ...
  options.material =
    options.material ||
    new THREE.MeshBasicMaterial({ color: 0xee6633 });
  ...
  var zs = THREE.Terrain.toArray1D(
    mesh.geometry.attributes.position.array,
  );
  if (
    options.heightmap instanceof HTMLCanvasElement ||
    options.heightmap instanceof Image
  ) {
...
THREE.Terrain.fromArray1D = function (vertices, src) {
  for (
    var i = 0, l = Math.min(vertices.length / 3, src.length);
    i < l;
    i++
  ) {
...
THREE.Terrain.heightmapArray = function (method, options) {
  var arr = new Array(
      (options.xSegments + 1) * (options.ySegments + 1),
    ),
    l = arr.length,
    i;
  arr.fill(0);
  ...
  options.maxHeight =
    typeof options.maxHeight === 'undefined' ? 1 : options.maxHeight;
  ...
  for (var row = 0; row < rows; row++) {
    for (var col = 0; col < cols; col++) {
      var i = row * cols + col,
        idx = i * 4;
      g[i] =
        ((data[idx] + data[idx + 1] + data[idx + 2]) / 765) * spread +
        options.minHeight;
    }
  }
};
  ...
  for (var row = 0; row < rows; row++) {
    for (var col = 0; col < cols; col++) {
      ...
      data[idx] =
        data[idx + 1] =
        data[idx + 2] =
          Math.round(((g[i * 3 + 2] - min) / spread) * 255);
      data[idx + 3] = 255;
    }
  }
...
THREE.Terrain.Clamp = function (g, options) {
  ...
  var actualRange = max - min,
    optMax =
      typeof options.maxHeight !== 'number' ? max : options.maxHeight,
    optMin =
      typeof options.minHeight !== 'number' ? min : options.minHeight,
    targetMax = options.stretch
      ? optMax
      : max < optMax
      ? max
      : optMax,
    targetMin = options.stretch
      ? optMin
      : min > optMin
      ? min
      : optMin,
  ...
  for (i = 0; i < l; i++) {
    g[i] =
      options.easing((g[i] - min) / actualRange) * range + optMin;
  }
};
...
THREE.Terrain.Edges = function (
  ...
  var numXSegments =
      Math.floor(distance / (options.xSize / options.xSegments)) || 1,
    numYSegments =
      Math.floor(distance / (options.ySize / options.ySegments)) || 1,
 ...
 *   between 0 and 1 and returns a float between 0 and 1.
 */
THREE.Terrain.RadialEdges = function (
  g,
  options,
  direction,
  distance,
  ...
  var peak = direction ? options.maxHeight : options.minHeight,
    ...
    edgeRadius =
      Math.min(options.xSize, options.ySize) * 0.5 - distance,
    i,
    j,
    multiplier,
    k,
...
THREE.Terrain.Smooth = function (g, options, weight) {
  ...
  for (
    var i = 0, xl = options.xSegments + 1, yl = options.ySegments + 1;
    i < xl;
    i++
  ) {
    for (var j = 0; j < yl; j++) {
  ...
  for (
    ...
    for (var j = 0; j < yl; j++) {
      ...
      if (neighborKeys.length % 2 === 1) {
        ...
        median =
          (g[neighborKeys[halfKey - 1]] + g[neighborKeys[halfKey]]) *
          0.5;
      }
...
THREE.Terrain.Step = function (g, levels) {
  ...
  for (i = 0; i < l; i++) {
    ...
    for (j = 0; j < levels; j++) {
      if (
        startHeight >= buckets[j].min &&
        startHeight <= buckets[j].max
      ) {
        g[i] = buckets[j].avg;
        break;
      }
...
THREE.Terrain.Turbulence = function (g, options) {
  ...
  for (var i = 0, l = g.length; i < l; i++) {
    g[i] =
      options.minHeight +
      Math.abs((g[i] - options.minHeight) * 2 - range);
  }
};
...
THREE.Terrain.MultiPass = function (g, options, passes) {
  ...
  for (var i = 0, l = passes.length; i < l; i++) {
    var amp =
        typeof passes[i].amplitude === 'undefined'
          ? 1
          : passes[i].amplitude,
    ...
    clonedOptions.frequency =
      typeof passes[i].frequency === 'undefined'
        ? options.frequency
        : passes[i].frequency;
...
THREE.Terrain.DiamondSquare = function (g, options) {
  ...
  var segments = THREE.MathUtils.ceilPowerOfTwo(
    Math.max(options.xSegments, options.ySegments) + 1,
  );
...
THREE.Terrain.Fault = function (g, options) {
  var d = Math.sqrt(
      options.xSegments * options.xSegments +
        options.ySegments * options.ySegments,
    ),
    ...
    smoothDistance =
      Math.min(
        options.xSize / options.xSegments,
        options.ySize / options.ySegments,
      ) * options.frequency;
...
(function () {
  ...
  THREE.Terrain.Particles = function (g, options) {
    var iterations =
      ...
      xl = options.xSegments + 1,
      displacement =
        ((options.maxHeight - options.minHeight) / iterations) * 1000,
...
THREE.Terrain.Simplex = function (g, options) {
  ...
  var range = (options.maxHeight - options.minHeight) * 0.5,
    divisor =
      ((Math.min(options.xSegments, options.ySegments) + 1) * 2) /
      options.frequency;
  for (var i = 0, xl = options.xSegments + 1; i < xl; i++) {
    for (var j = 0, yl = options.ySegments + 1; j < yl; j++) {
      g[j * xl + i] +=
...
THREE.Terrain.generateBlendedMaterial = function (textures) {
  ...
  for (var i = 0, l = textures.length; i < l; i++) {
    // Update textures
    textures[i].texture.wrapS = textures[i].wrapT =
...
THREE.Terrain.ScatterMeshes = function (geometry, options) {
  if (!options.mesh) {
    console.error(
      'options.mesh is required for THREE.Terrain.ScatterMeshes but was not passed',
    );
    return;
  }
  if (!options.scene) {
  ...
  for (
    ...
    if (place) {
      ...
      if (options.sizeVariance) {
        var variance =
          Math.random() * doubleSizeVariance - options.sizeVariance;
 ...
 *   function.
 */
THREE.Terrain.ScatterHelper = function (
  method,
  options,
  skip,
  threshold,
...
THREE.Terrain.Influences = {
  ...
  Volcano: function (x) {
    return (
      0.94 -
      0.32 *
        (Math.abs(2 * x) + Math.cos(2 * Math.PI * Math.abs(x) + 0.4))
    );
  },
};
...
THREE.Terrain.Influence = function (g, options, f, x, y, r, h, t, e) {
  ...
  for (var i = xs; i < xe; i++) {
    for (var j = ys; j < ye; j++) {
      ...
      if (t === THREE.AdditiveBlending)
        g[k] += d; // jscs:ignore requireSpaceAfterKeywords
      else if (t === THREE.SubtractiveBlending) g[k] -= d;
      else if (t === THREE.MultiplyBlending) g[k] *= d;
      else if (t === THREE.NoBlending) g[k] = d;
      else if (t === THREE.NormalBlending)
        g[k] = e(fdr, fdxr, fdyr) * g[k] + d;
      else if (typeof t === 'function')
        g[k] = t(g[k].z, d, fdr, fdxr, fdyr);
    }
  }
};
